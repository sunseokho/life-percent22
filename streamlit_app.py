...
if st.button("Predict"):
    try:
        ...
        st.success(f"예상 사망확률 : {prediction*100:.2f}%")
    except Exception as e:
...
# 예측 결과에 따라 콘텐츠 표시
    if prediction >= 0.5:
        st.write("### 폐암 관련 컨텐츠.")
        st.image("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRP1OnNiwdiS2sev9VS9aVyqhqUAy_R44TuS4coyyko4w&s", caption="Low Segment Image 1")
        st.video("https://www.youtube.com/watch?v=AIkriQweg5k")  # YouTube 썸네일
        st.text("Description for Low Segment 1")
    if prediction <=0.5:
        st.write("### 폐암 관련 컨텐츠2.")
        st.image("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnJvSWMtI_yAoQTiz_vWbF3IQFe-5bjw4crQ&s")
        st.video("https://www.youtube.com/watch?v=CksOo3xyIgY")  # YouTube 썸네일
        st.text("Description for Low Segment 1")
